...
def populate_test_cases(
    modules: T.List[str]
) -> T.Generator[T.Tuple[str, unittest.TestCase], None, None]:
    for module_name in modules:
        ...
        for each_attr in attr_names:
            ...
            if isinstance(attr_val,
                          type) and issubclass(attr_val, unittest.TestCase):
...
def run_tests(tests: T.List[T.Tuple[str, unittest.TestCase]]) -> bool:
    ...
    for (_name, each_test) in tests:
        test_names = [
            prop for prop in dir(each_test) if prop.startswith("test_")
        ]
