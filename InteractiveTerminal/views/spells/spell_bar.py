...
class SpellIcon(Image, BoxSized, ListenForStateChanges, TouchableMixin, OptionalTooltip):
    ...
    def listener(self, ev: GameOrViewEvent, is_forward: bool, state_manager: StateManager) -> None:
        if state_manager.view_state.focused_character is None:
            ...
            self.spell_save_dc = 11 + char.stat_block[Stat.PROFICIENCY_BONUS] + char.stat_block[Stat.INTELLIGENCE]
            self.int_modifier = char.stat_block[Stat.INTELLIGENCE]
            self.spell_attack_bonus = char.stat_block[Stat.PROFICIENCY_BONUS] + char.stat_block[Stat.INTELLIGENCE]
    ...
    def on_left_click(self, position):
        ...
        self.state_manager.push_event(cast_spell(character_id=self.state_manager.view_state.focused_character, which_spell=self.which_spell))
    ...
    def on_right_click(self, position):
        ...
        self.state_manager.push_event(followup_spell(character_id=self.state_manager.view_state.focused_character, which_spell=self.which_spell))
