...
from ...new_models.events.game_events import (
    CastDeflect, ConsumeRevival, RestoreDeflect, RestoreHitDice, RestoreRevival,
    UseHitDice
)
...
class CHDeflections(ProgressiveIcon, TouchableMixin, ListenForStateChanges):
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(
            current_deflects=self.update_deflects,
            max_deflects=self.update_deflects
        )
    ...
    def update_deflects(self, *args):
        self.tooltip_text = help_generator.deflect(
            n_free_casts=self.current_deflects,
            max_free_casts=self.max_deflects,
...
class CHRevivals(ProgressiveIcon, TouchableMixin, ListenForStateChanges):
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(
            current_revives=self.revives_updated,
            max_revives=self.revives_updated
        )
    ...
    def revives_updated(self, *args):
        self.tooltip_text = help_generator.revive(
            self.current_revives, self.max_revives
        )
...
class CHHitDice(ProgressiveIcon, TouchableMixin, ListenForStateChanges):
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(
            current_hit_dice=self.update_hit_dice,
            max_hit_dice=self.update_hit_dice
        )
