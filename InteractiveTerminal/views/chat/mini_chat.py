    ...
    def listener(
        ...
        if len(state_manager.game_state.chat_log) == 0:
            ...
            if state_manager.view_state.chat_log_index < 0:
                if state_manager.view_state.chat_log_index >= -1 * len(
                    state_manager.game_state.chat_log
                ):
                    self.text = state_manager.game_state.chat_log[
                        state_manager.view_state.chat_log_index]
                else:
                    self.text = "NO MORE MESSAGES"
            else:
                if state_manager.view_state.chat_log_index < len(
                    state_manager.game_state.chat_log
                ):
                    self.text = state_manager.game_state.chat_log[
                        state_manager.view_state.chat_log_index]
